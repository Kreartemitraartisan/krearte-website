...
// =========================
// ✅ GET: Fetch single order by ID or Order Number
// =========================
export async function GET(
  ...
  try {
    ...
    console.log("🔍 Fetching order with ID/OrderNumber:", id);
    ...
    if (user?.role !== "admin") {
      console.warn("⚠️ Non-admin user tried to access order:", session.user.email);
      return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 });
    }
    
    // ✅ Support both database ID and orderNumber
    // Check if it looks like an order number (starts with KR-)
    const whereClause = id.startsWith('KR-') 
      ? { orderNumber: id }
      : { id };
...
export async function PATCH(
  ...
  try {
    ...
    // ✅ 3. Support both ID and orderNumber lookup
    const whereClause = id.startsWith('KR-') 
      ? { orderNumber: id }
      : { id };

    // ✅ 4. Check if order exists first
    ...
    const updatedOrder = await prisma.order.update({
      where: { id: existingOrder.id }, // Use database ID for update
      data: updateData,
    });

    // ✅ 6. Send webhook to n8n if status changed (non-blocking)
    ...
    console.log(`✅ Order ${existingOrder.orderNumber} updated:`, updateData);
...
export async function DELETE(
  ...
  try {
    ...
    // ✅ Support both ID and orderNumber lookup
    const whereClause = id.startsWith('KR-') 
      ? { orderNumber: id }
      : { id };
