...
function isPhysicalMaterial(material: any): boolean {
  ...
  const excludedKeywords = ["service", "add-on", "addon", "jasa", "print", "design", "redesign", "sample"];
    ...
    const productsWithPriceRange = await Promise.all(
      products.map(async (product) => {
        try {
          ...
          const prices = physicalMaterials.map((m) => (m.pricePerM2 || 0) + (m.waste || 0));
          ...
          return { ...product, priceRange: { min: product.price || 0, max: product.price || 0 } };
        }
      })
    );
